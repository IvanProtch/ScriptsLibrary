...
public class Scripto
{
    ...
    public void Execute(IActivity activity)
    {
        ...
        List<long> userGroupIDs = new List<long>() { 3254492/*Все пользователи БМЗ*/ /*Все пользователи КСК*/};
        //long userGroupID = 3254492; // +++++++++++++++++++++++++++++++++++++ ПРОПИСЫВАЕМЫЙ ПОЛЬЗОВАТЕЛЬ\ГРУППА - Все пользователи БМЗ
        ...
        foreach (long ecoID in ecoIDs)
        {
            ...
            foreach (long objContxtID in contextIds)
            {
                ...
                foreach (DataRow dataRow in dtAccList.Rows)
                {
                    if (userGroupIDs.Contains(Convert.ToInt64(dataRow[Consts.F_USER_ID])) && dataRow[Consts.F_RIGHT_ID].ToString() == "8" && dataRow[Consts.F_RIGHT_TYPE].ToString() == "2")
                    {
                        t++;
                        break;
                    }
