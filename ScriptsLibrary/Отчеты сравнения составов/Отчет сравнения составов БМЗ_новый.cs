...
namespace EcoDiffReport
{
    ...
    public class Relation
    {
        ...
        public IDictionary<long, MeasuredValue> GetAmount(ref bool hasemptyAmountRelations, Tuple<string, string> exceptionInfo, Item endItem)
        {
    ...
    public class Item
    {
        ...
        /// <summary>
        /// Причина добавления в извещение
        /// </summary>
        public ReasonToAddInECO ReasonToAdd = ReasonToAddInECO.edit;
        /// <summary>
        ...
        public IDictionary<long, MeasuredValue> GetAmount(ref bool hasemptyAmountRelations, Tuple<string, string> exceptionInfo, Item endItem)
        {
            ...
            foreach (Relation relation in RelationsWithParent)
            {
                ...
                IDictionary<long, MeasuredValue> itemsAmount = relation.GetAmount(ref hasemptyAmountRelations, exceptionInfo, endItem);
    ...
    /// Заготовка детали
    /// </summary>
    public class BlankMaterial : Item
    {
        protected Dictionary<long, Item> _connectedItemsAndSelf = new Dictionary<long, Item>();
        public Dictionary<long, Item> ConnectedItemsAndSelf
        {
            get
            {
                if (_connectedItemsAndSelf.Count == 0)
                    _connectedItemsAndSelf[this.ObjectId] = this.Clone();
    ...
    /// <summary>
    /// Комплектующая единица
    /// </summary>
    public class СomponentUnit : Item
    {
        private string _blanksOrganization;
        public Item AssociatedPart { get; set; }

        public СomponentUnit(string blanksOrganization) : base()
        {
            _blanksOrganization = blanksOrganization;
        }
        /// <summary>
        /// Есть заготовка из организации
        /// </summary>
        /// <param name="org"></param>
        /// <returns></returns>
        public bool HasBlankFromOrg
        {
            get
            {
                ...
                var relWithMO = AssociatedPart.RelationsWithChild.FirstOrDefault();
                var partMO = relWithMO != null ? relWithMO.Child : null;
                if (partMO != null)
                {
                    var relWithBlank = partMO.RelationsWithChild.FirstOrDefault();
                    var blank = relWithBlank != null ? relWithBlank.Child : null;

                    if (blank != null)
                    {
                        if (blank is BlankMaterial)
                            return blank.SourseOrg == _blanksOrganization;
                    }
                }
                return false;
            }
        ...
        public class Report : ILogFile
    {
        ...
        private class ReportRow
        {
            ...
            private Dictionary<string, Tuple<MeasuredValue, MeasuredValue>> GetAmountInAsm(Item itemToInit)
            {
                ...
                if (itemToInit is BlankMaterial)
                    items.AddRange((itemToInit as BlankMaterial).ConnectedItemsAndSelf.Values);
                else
                ...
                foreach (var item in items)
                {
                    foreach (var asm in item.EntersInAsms)
                    {
                        ...
                        var itemAmount = item.GetAmount(ref item.HasEmptyAmount, null, asm.Value).Values.FirstOrDefault();
                        ...
                        if (entersInAsmToInit.ContainsKey(asm.Value.Caption) && itemAmount != null)
                            ...
                            entersInAsmToInit[asm.Value.Caption] = new Tuple<MeasuredValue, MeasuredValue>(itemAmount == null ? MeasureHelper.ConvertToMeasuredValue("0 шт") : itemAmount, asmAmount == null ? MeasureHelper.ConvertToMeasuredValue("0 шт") : asmAmount);
                    }
                }
            ...
            //private MeasuredValue _emptyAmount = MeasureHelper.ConvertToMeasuredValue("0 шт");

            public Item BaseItem { get; set; }
            public Item ECOItem { get; set; }
            ...
            public MeasuredValue Amount_base
            {
                ...
                {
                    if (BaseItem == null)
                        return MeasureHelper.ConvertToMeasuredValue("0 шт");

                    if (_amount_base == null)
                    {
                        _amount_base = BaseItem == null ? MeasureHelper.ConvertToMeasuredValue("0 шт") : BaseItem.AmountSum;
                    }
                    return _amount_base;
                }
                set { }
            ...
            public MeasuredValue Amount_eco
            {
                get
                {
                    ...
                    if (_amount_eco == null)
                    {
                        _amount_eco = ECOItem == null ? MeasureHelper.ConvertToMeasuredValue("0 шт") : ECOItem.AmountSum;
                    }
                    return _amount_eco;
                }
                set { }
        ...
        {
            private List<ReportRow> _rowList = new List<ReportRow>();
            private Dictionary<Tuple<long, long, string>, Item> _baseComposition;
            private Dictionary<Tuple<long, long, string>, Item> _ecoComposition;
            private IUserSession _session;
            private ILogFile _logger;
            ...
            public ReportTable(Dictionary<Tuple<long, long, string>, Item> baseComposition, Dictionary<Tuple<long, long, string>, Item> ecoComposition, IUserSession session, ILogFile log)
            {
                _session = session;
                _baseComposition = baseComposition;
                _ecoComposition = ecoComposition;
                _logger = log;
                Init();

                _logger.WriteMessage("После Init()");
                _rowList.ForEach(e => _logger.WriteMessage(e.ToString()));

                var contextItems = _rowList.Where(e => (e.ECOItem != null && e.ECOItem.isContextObject) || (e.BaseItem != null && e.BaseItem.isContextObject));
                foreach (var row in contextItems)
                {
                    Item item = row.ECOItem == null ? row.BaseItem : row.ECOItem;
                    ChangeCompositionFrom(item, item.ReasonToAdd);
                }
                _rowList = _rowList.Except(contextItems).ToList();

                _logger.WriteMessage("После ChangeCompositionFrom()");
                _rowList.ForEach(e => _logger.WriteMessage(e.ToString()));

                _rowList = _rowList.Where(e => (!e.Amount_base.Equals(e.Amount_eco)) || e.ReplacementGroupIsChanged || e.ReplacementStatusIsChanged).ToList();

                _logger.WriteMessage("После Where(e => (!e.Amount_base.Equals(e.Amount_eco))");
                _rowList.ForEach(e => _logger.WriteMessage(e.ToString()));

                //вычисление изменений в группах заменителей
                ReplacementGroupCalculation();

                RefreshData();
                _rowList.OrderBy(e => e.MaterialCaption);

                _logger.WriteMessage("После RefreshData()");
                _rowList.ForEach(e => _logger.WriteMessage(e.ToString()));

                //добавление в конец таблицы последовательности материалов из заготовок+всех связанных с ней деталей, которые должны попасть в отчет
                AppendBlankMaterialsAndConnectedParts();

            }

            private void Init()
            {
                foreach (var resItem in _baseComposition)
                {
                    Item ecoItem;
                    //Находим базовый объект в списке объектов из извещения
                    if (_ecoComposition.TryGetValue(resItem.Key, out ecoItem))
                    {
                        _ecoComposition.Remove(resItem.Key);
                    }
                    else
                    {
                        var emptyItem = new Tuple<long, long, string>(0, resItem.Key.Item2, string.Empty);
                        if (_ecoComposition.TryGetValue(emptyItem, out ecoItem))
                        {
                            _ecoComposition.Remove(emptyItem);
                        }
                    }

                    ReportRow material = new ReportRow(resItem.Value, ecoItem);
                    _rowList.Add(material);
                }
                foreach (var ecoItem in _ecoComposition.Values)
                {
            ...
            /// <summary>
            /// Меняет состав item с учетом причины добавления в ИИ:
            /// если у item причина включения = создание, baseItem=null, ecoItem=item.Amount;
            /// если у item причина включения = аннулирование, baseItem=item.Amount, ecoItem=null.
            /// </summary>
            /// <param name="item"></param>
            private void ChangeCompositionFrom(Item item, Item.ReasonToAddInECO reasonToAddInECO)
            {
                foreach (var relationWithChild in item.RelationsWithChild)
                {
                    if (relationWithChild.Child.MaterialId > 0)
                    {
                        var child = _rowList.FirstOrDefault(e => (e.BaseItem != null && e.BaseItem.ObjectId == relationWithChild.Child.ObjectId) || (e.ECOItem != null && e.ECOItem.ObjectId == relationWithChild.Child.ObjectId));

                        if (child != null)
                        {
                            if (child.ECOItem == null)
                                continue;

                            if (reasonToAddInECO == Item.ReasonToAddInECO.create)
                            {
                                child.BaseItem = null;
                                child.Amount_base = null;
                            }
                            else if (reasonToAddInECO == Item.ReasonToAddInECO.delete)
                            {
                                child.ECOItem = null;
                                child.Amount_eco = null;
                            }
                        }
                    }
                    else
                        ChangeCompositionFrom(relationWithChild.Child, reasonToAddInECO);
                }
            }

            /// <summary>
            /// Добавление в таблицу последовательности материалов из заготовок+всех связанных с ней деталей, которые должны попасть в отчет
            /// </summary>
            private void AppendBlankMaterialsAndConnectedParts()
            {
                ...
                foreach (var zgMat in _rowList.Where(e => e.BaseItem is BlankMaterial || e.ECOItem is BlankMaterial))
                {
                    var zagBaseComp = zgMat.BaseItem != null ? new Dictionary<long, Item>((zgMat.BaseItem as BlankMaterial).ConnectedItemsAndSelf) : new Dictionary<long, Item>();
                    var zagEcoComp = zgMat.ECOItem != null ? new Dictionary<long, Item>((zgMat.ECOItem as BlankMaterial).ConnectedItemsAndSelf) : new Dictionary<long, Item>();
                    ...
                    foreach (var zag in zags.Where(e => !e.Amount_base.Equals(e.Amount_eco)))
                    {
                        ...
                        if (partEco != null)
                        {
                            ...
                            partEco.AmountSum = partEco.GetAmount(ref partEco.HasEmptyAmount, exceptionInfo, null).Values.FirstOrDefault();
                        ...
                        if (partBase != null)
                        {
                            ...
                            partBase.AmountSum = partBase.GetAmount(ref partBase.HasEmptyAmount, exceptionInfo, null).Values.FirstOrDefault();
                        }
                ...
                _rowList.RemoveAll(e => e.BaseItem is BlankMaterial || e.ECOItem is BlankMaterial);

                //добавляем новые детали в итоговую выборку
                foreach (var item in matPartPairs)
                {
                    _rowList.AddRange(item.Value);
                    _rowList.Add(item.Key);
                }

                #endregion
            }

            private void ReplacementGroupCalculation()
            {
                ...
                foreach (var item in _rowList)
                {
                    if (item.ReplacementGroupIsChanged || item.ReplacementStatusIsChanged)
                    {
                        ...
                        if (item.BaseItem.isActualReplacement && item.ECOItem.isPossableReplacement)
                        {
                            ...
                            var actual = _rowList.FirstOrDefault(e => e.ECOItem.isActualReplacement && e.ECOItem.ReplacementGroup == item.ECOItem.ReplacementGroup);
                        ...
                        if (item.BaseItem.isPossableReplacement && item.ECOItem.isActualReplacement)
                        {
                            ...
                            var possable = _rowList.FirstOrDefault(e => e.ECOItem.isPossableReplacement && e.ECOItem.ReplacementGroup == item.ECOItem.ReplacementGroup);
            ...
            {
                List<long> resultCompIds = _rowList
                ...
                resultCompIds.AddRange(
                    _rowList
                    .Where(e => e.BaseItem is BlankMaterial || e.ECOItem is BlankMaterial)
                ...
                resultComposition_tech.AddRange(_rowList.Where(e => e.IsPurchased));

                if (resultCompIds.Count > 0)
                {
                    IDBObjectCollection col = _session.GetObjectCollection(-1);
                    ...
                    columns.Add(new ColumnDescriptor(MetaDataHelper.GetAttributeTypeID(new Guid("8debd174-928c-4c07-9dc1-423557bea1d7" /*Признак изготовления БМЗ*/)), AttributeSourceTypes.Object, ColumnContents.Text,
                    ColumnNameMapping.Guid, SortOrders.NONE, 0));

                    columns.Add(new ColumnDescriptor(MetaDataHelper.GetAttributeTypeID("84ffec95-9b97-4e83-b7d7-0a19833f171a" /*Организация-источник*/), AttributeSourceTypes.Object, ColumnContents.Text,
                    ColumnNameMapping.Guid, SortOrders.NONE, 0));

                    HybridDictionary tags = new HybridDictionary();

                    var dbrsp = new DBRecordSetParams(conds.ToArray(),
                    columns != null
                    ? columns.ToArray()
                    : null,
                    0, null,
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        var materials = _rowList.Where(x => (x.LinkToObjId == id) || x.MaterialId == id);
                        foreach (var mat in materials)
                        {
                            bool hasBlank = false;
                            var item = mat.BaseItem == null ? mat.ECOItem : mat.BaseItem;
                            if (item is СomponentUnit)
                                hasBlank = (item as СomponentUnit).HasBlankFromOrg;

                            if (mat != null && (isPurchased || isCoop || hasBlank))
                            {
                                mat.IsPurchased = isPurchased;
                                mat.MaterialCode = code;
                                if (mat.BaseItem is BlankMaterial || mat.ECOItem is BlankMaterial)
                ...
                #endregion
                _rowList = resultComposition_tech;
            ...
            private void RemoveRepeats()
            {
                ...
                foreach (var row in _rowList)
                {
                    ...
                    {
                        if (repItem.MaterialCaption == row.MaterialCaption && repItem.Type == row.Type && repItem.MeasureUnit == row.MeasureUnit)
                            repeatItem = repItem;
                    }

                    if (repeatItem != null)
                        repeatItem.Combine(row);
                    else
                        reportComp.Add(row);
                }
        ...
        private class ReportWordDocument
        {
            ...
            private ReportTable _resultComposition;
            private bool _compliteReport;

            public ReportWordDocument(ImDocumentData document, ReportTable resultComposition, string ecoCaption, string asmCaption, bool completeReport)
            {
                _document = document;
            ...
            public void WriteReport()
            {
                ...
                foreach (var item in _resultComposition.Rows)
                {
        ...
        private Item GetItem(DataRow row, Tuple<Dictionary<long, Item>, Dictionary<long, Item>> itemsDict, bool contextMode, CompositionType compositionType)
        {
            ...
            else
            {
                ...
                else if (objType == _zagotType)
                    item = new BlankMaterial();
                else if (objType == _complectUnitType)
                    item = new СomponentUnit(blankOrg);
                else
                    item = new Item();
                ...
                if (contextMode)
                {
                    if (contextObjectsID.Contains(item.Id))
                    {
                        ...
                        IDBRelation rel = _session.GetRelation(_eco, item.Id);
                        IDBAttribute reasonToAdd = null;

                        if(rel != null)
                            reasonToAdd = rel.GetAttributeByGuid(new Guid("cad007a3-306c-11d8-b4e9-00304f19f545" /*Цель включения объекта*/));

                        if (reasonToAdd != null)
                        {
                            item.ReasonToAdd = reasonToAdd.Description == "Создание" ? Item.ReasonToAddInECO.create : reasonToAdd.Description == "Аннулирование" ? Item.ReasonToAddInECO.delete : Item.ReasonToAddInECO.edit;
                        }
        ...
        private Dictionary<Tuple<long, long, string>, Item> GetComposition(IDBObject headerObj, DataTable dtCompos, IUserSession session, CompositionType compositionType = CompositionType.technology)
        {
            ...
            header.Description = headerObj.GetAttributeByGuid(new Guid(SystemGUIDs.attributeDesignation)).AsString;
            ...
            foreach (var item in itemsDict.Item1.Values)
            {
                ...
                if (item.ObjectType == _partType || item.ObjectType == _CEType)
                {
                    ...
                    IEnumerable<string> itemEntersInAsms = item.EntersInAsms.Values.Select(e => e.Description);

                    Item associatedComplectUnit = itemsDict.Item2.Values
                        .OfType<СomponentUnit>()
                        .Where(e => e.Caption == item.Caption)
                        .FirstOrDefault(e => e.EntersInAsms.Values.Select(r => r.Description).Where(t => itemEntersInAsms.Contains(t)).Count() > 0);
                    ...
                    if (associatedComplectUnit != null)
                    {
                        ...
                        //добавляем ассоциацию для комплектующей
                        if (associatedComplectUnit is СomponentUnit)
                            (associatedComplectUnit as СomponentUnit).AssociatedPart = item;
                    }
            ...
            foreach (Item item in itemsDict.Item1.Values)
            {
                ...
                if (item.isContextObject)
                {
                    Tuple<long, long, string> itemKey = new Tuple<long, long, string>(
                                           0,
                                           item.MaterialId != 0 ? item.MaterialId : item.ObjectId,
                                           item.SourseOrg);
                    if (!composition.ContainsKey(itemKey))
                    {
                        composition[itemKey] = item;
                    }
                }
                ...
                var itemsAmount = item.GetAmount(ref item.HasEmptyAmount, exceptionInfo, null);
                ...
                foreach (var itemAmount in itemsAmount)
                {
                    ...
                    item.AmountSum = itemAmount.Value.Clone() as MeasuredValue;
                    ...
                    if (composition.TryGetValue(itemKey, out actualItem))
                    {
                        ...
                        {
                            if(actualItem is BlankMaterial)
                                (actualItem as BlankMaterial).ConnectedItemsAndSelf[item.ObjectId] = item;

                            actualItem.AmountSum.Add(item.AmountSum.Clone() as MeasuredValue);

                            //обновляем данные по входямостям в сборки
                            foreach (var newItem in item.EntersInAsms)
                            {
                            ...
                            actualItem.AmountSum = item.AmountSum.Clone() as MeasuredValue;
                    }
                    else
                    {
                        composition[itemKey] = item;
            ...
            {
                asm.AmountSum = asm.GetAmount(ref asm.HasEmptyAmount, null, null).Values.FirstOrDefault();
            }
        ...
        {
            if(!testMode)
                _messageRecepients = new List<long>() { 3252010 /*Булычева ЕИ*/, 62180376 /*Бородина ЕВ*/ }; //  51448525 /*Протченко ИВ*/
            ...
            WriteMessage(string.Format("Отчет для: {0} {1}", headerObj.NameInMessages, ecoObj.NameInMessages));
            ...
            WriteMessage("DataTable1");
            WriteMessage(dt.Rows.Count.ToString());
            ...
            WriteMessage("Первый состав по извещению");
            ecoComposition.Values.ToList().ForEach(e => WriteMessage(e.ToString()));
            ...
            WriteMessage("Второй состав без извещения");
            baseComposition.Values.ToList().ForEach(e => WriteMessage(e.ToString()));

            //возвращаем контекст
            session.EditingContextID = sessionid;

            //Итоговый состав материалов
            ReportTable reportTable = new ReportTable(baseComposition, ecoComposition, session, this as ILogFile);

            WriteMessage("Итоговый состав материалов");
            reportTable.Rows.ForEach(e => WriteMessage(e.ToString()));

            foreach (var item in reportTable.Rows)
            {
            ...
            //Формирование отчета
            ReportWordDocument doc = new ReportWordDocument(document, reportTable, ecoObjCaption, headerObjCaption, compliteReport);
