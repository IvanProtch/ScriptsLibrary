...
namespace EcoDiffReport2
{
    ...
    public class Report
    {
        ...
        public bool Run(IUserSession session, ImDocumentData document, Int64[] objectIDs)
        {
            ...
            // состав по извещен
            Composition ecoComposition = new Composition(session, dt, headerObjBase, checkAmountTypes);
            ...
            Composition baseComposition = new Composition(session, dt, headerObjBase, checkAmountTypes);
            ...
            foreach (Item item in ecoComposition.Values)
            {
                Material mat = new Material();
    ...
    public class Composition : IDictionary<Tuple<long, long>, Item>
    {
        private IUserSession _session;
        private IDBObject _headerObject;
        private DataTable _compositionData;
        private List<int> _checkAmountTypes;
        ...
        public string UserError = string.Empty;
        public string AdminError = string.Empty;
        ...
        /// <summary>
        /// Кортеж словарей в который будут заноситься objectid и Item; linkToObjId и Item. Организует иерархическую связь между Item.</param>
        /// </summary>
        private Tuple<Dictionary<long, Item>, Dictionary<long, Item>> _itemsDict;
        ...
        /// <returns></returns>
        private Item GetItem(DataRow row, bool contextMode)
        {
            Item item = null;
            ...
            var itemsDict_byObjId = _itemsDict.Item1;
            var itemsDict_byLinkId = _itemsDict.Item2;
            ...
            {
                foreach (var checkAmountType in _checkAmountTypes)
                {
        ...
        private void InitComposition(IDBObject headerObj, DataTable dtCompos, IUserSession session)
        {
            ...
            _composition = new Dictionary<Tuple<long, long>, Item>();
            _itemsDict = new Tuple<Dictionary<long, Item>, Dictionary<long, Item>>(new Dictionary<long, Item>(), new Dictionary<long, Item>());
            ...
            _itemsDict.Item1[header.ObjectId] = header;
            ...
            foreach (DataRow row in dtCompos.Rows)
            {
                ...
                var item = GetItem(row, true);
                //if (item != null)
                //    AddToLog("item " + item.ToString());
            }

            _itemsDict.Item1.Remove(headerObj.ObjectID);

            foreach (var item in _itemsDict.Item1.Values)
            {
                ...
                if (item.ObjectType == _asmUnitType)
                {
                    ...
                    if (_itemsDict.Item2.TryGetValue(item.LinkToObjId, out complectUnit))
                    {
                ...
                if (item.ObjectType == _partType || item.ObjectType == _CEType)
                {
                    ...
                    var associatedComplectUnit = _itemsDict.Item2.Values
            ...
            foreach (var item in _itemsDict.Item1.Values)
            {
                ...
                foreach (var itemAmount in itemsAmount)
                {
                    ...
                    if (_composition.TryGetValue(itemKey, out cachedItem))
                    {
                    ...
                    {
                        _composition[itemKey] = AmountItemClone;
                    }
                }
            }
        }

        public Composition(IUserSession session, DataTable compositionData, IDBObject headerObject, List<int> checkAmountTypes)
        {
            this._session = session;
            this._compositionData = compositionData;
            this._headerObject = headerObject;
            this._checkAmountTypes = checkAmountTypes;
            ...
            InitComposition(headerObject, compositionData, session);
        }

        private Dictionary<Tuple<long, long>, Item> _composition;

        public ICollection Keys => _composition.Keys;

        public ICollection Values => _composition.Values;

        public int Count => _composition.Count;

        ICollection<Tuple<long, long>> IDictionary<Tuple<long, long>, Item>.Keys => ((IDictionary<Tuple<long, long>, Item>)_composition).Keys;

        ICollection<Item> IDictionary<Tuple<long, long>, Item>.Values => ((IDictionary<Tuple<long, long>, Item>)_composition).Values;

        public bool IsReadOnly => ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).IsReadOnly;

        public Item this[Tuple<long, long> key] { get => _composition[key]; set => _composition[key] = value; }
        ...
        public bool ContainsKey(Tuple<long, long> key)
        {
            return ((IDictionary<Tuple<long, long>, Item>)_composition).ContainsKey(key);
        }

        public void Add(Tuple<long, long> key, Item value)
        {
            ((IDictionary<Tuple<long, long>, Item>)_composition).Add(key, value);
        }

        public bool Remove(Tuple<long, long> key)
        {
            return ((IDictionary<Tuple<long, long>, Item>)_composition).Remove(key);
        }

        public bool TryGetValue(Tuple<long, long> key, out Item value)
        {
            return ((IDictionary<Tuple<long, long>, Item>)_composition).TryGetValue(key, out value);
        }

        public void Add(KeyValuePair<Tuple<long, long>, Item> item)
        {
            ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).Add(item);
        }

        public bool Contains(KeyValuePair<Tuple<long, long>, Item> item)
        {
            return ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).Contains(item);
        }

        public void CopyTo(KeyValuePair<Tuple<long, long>, Item>[] array, int arrayIndex)
        {
            ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).CopyTo(array, arrayIndex);
        }

        public bool Remove(KeyValuePair<Tuple<long, long>, Item> item)
        {
            return ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).Remove(item);
        }

        IEnumerator<KeyValuePair<Tuple<long, long>, Item>> IEnumerable<KeyValuePair<Tuple<long, long>, Item>>.GetEnumerator()
        {
            return ((IEnumerable<KeyValuePair<Tuple<long, long>, Item>>)_composition).GetEnumerator();
        }

        public void Clear()
        {
            ((ICollection<KeyValuePair<Tuple<long, long>, Item>>)_composition).Clear();
        }
    }
