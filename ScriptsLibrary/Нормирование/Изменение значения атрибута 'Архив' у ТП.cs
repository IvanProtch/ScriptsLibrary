    ...
    public class Script
{
    ...
    #region Константы
    /// <summary>
    /// Значение атрибута 'Архив'
    /// </summary>
    const int archiveValue = 42715079; // УГТ общие документы
            ...
            foreach (long techII in techIIs)
            {
                ...
                foreach (long techprocess in techprocesses)
                {
                    ...
                    if (FromBMZ(UserSession, techprocess) == true)
                    {
                        try
                        {
                            IDBAttribute archive = techprocessObj.GetAttributeByName("Архив");
                            if (Convert.ToInt64(archive.Value) != archiveValue)
                            {
